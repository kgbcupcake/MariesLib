...
public final class LegendComponent implements MarieComponent, SelfPositioningModule {
    ...
    /** One column of the legend: a colored swatch, two lines of text, and small per-entry pixel offsets (in local units, before scale) preserved from the original hand-tuned Diet Screen layout. */
    ...
    private void drawEntry(RenderContext context, int x, int y, int w, double scale, LegendEntry entry, float fscale) {
        ...
        int squareSize = Math.max(1, (int) Math.round(8 * scale));
        int squareX = x + (w / 2) - (int) Math.round(18 * scale);
        context.fillRect(squareX, y + (int) Math.round(scale), squareSize, squareSize, accent);

        String line1 = entry.line1().get();
        String line2 = entry.line2().get();
        int line1X = squareX + (int) Math.round(11 * scale) + (int) Math.round(entry.line1OffsetX() * scale);
        int line2X = x + ((w - context.textWidth(line2, fscale)) / 2) + (int) Math.round(6 * scale) + (int) Math.round(entry.line2OffsetX() * scale);
        context.drawText(line1, line1X, y, dimLegend(textColorSupplier.getAsInt()), fscale);
